...
const RULES: PatternRule[] = [
  {
    ...
    pattern: /\b(ignore|disregard|forget|override)\b.{0,48}\b(previous|prior|above|system|developer|instructions?|rules?)\b/is,
    score: 55,
    severity: "critical",
  },
  {
    ...
    pattern: /\b(system|developer|assistant)\s*(prompt|message|instruction)|\bprompt\s*injection\b/is,
    score: 35,
    severity: "high",
  },
  {
    ...
    pattern: /\b(reveal|print|return|send|exfiltrate|leak|show)\b.{0,56}\b(secret|token|password|credential|api[ _-]?key|environment variable|system prompt)\b/is,
    score: 55,
    severity: "critical",
  },
  {
    ...
    pattern: /\b(run|execute|call|invoke|open|browse|fetch|curl|wget)\b.{0,48}\b(tool|command|shell|terminal|url|https?:\/\/|endpoint)\b/is,
    score: 40,
    severity: "high",
  },
  {
    ...
    pattern: /\b(these are|this is|follow)\b.{0,40}\b(trusted|authorized|highest priority|new instructions?|admin instructions?)\b/is,
    score: 25,
    severity: "medium",
  },
  {
    ...
    pattern: /\b(you must|must now|do not mention|respond only|instead,? (?:you|do)|complete the following)\b/is,
    score: 20,
    severity: "medium",
  },
];
